...
- **Skill discovery.** Claude Code auto-discovers skills from `.claude/skills/`, which is a *generated mirror* of the canonical `skills/` source. After changing anything under `skills/`, run `python scripts/sync_skills.py` so Claude Code (and the Copilot/Codex `.agents/skills/` mirror) pick up the change. Never edit the mirror directly — edit `skills/`.
- **Plugin install.** This repo is also a Claude Code plugin (`.claude-plugin/`). To install the family into another project: `/plugin marketplace add johnsonice/RA-Skills` then `/plugin install imf-ra-skills`.
